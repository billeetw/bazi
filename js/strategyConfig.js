/**
 * 伯彥戰略庫 (Strategy Library)
 * 12 宮位 × 4 種強度（4=主場, 3=穩定, 2=瓶頸, 1=考場）+ 4 種四化（祿、權、科、忌）
 * 宮位 key 與 PALACE_DEFAULT 一致：命宮、兄弟、夫妻、子女、財帛、疾厄、遷移、僕役、官祿、田宅、福德、父母
 */
(function (global) {
  "use strict";
  ...
  /**
   * 語法結構：[環境描述] + [轉折詞] + [行動指令]
   * 地形好(3,4)+吉化 → 「。在此基礎上」；地形差(1,2)+吉化 → 「。所幸」；化忌 → 「。但預警：」「。可惜」
   * 若有多個四化，優先只顯示「忌」。
   */
  function generateAdvice(palace, strength, sihuaList) {
